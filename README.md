...
| 前端 | 埠號 | 服務者 | 建置方式 |
|---|---|---|---|
| **管理後台** `/admin/` | 8080 | Flask（經主 nginx） | `docker compose build api` |
| **玩家站** `/` | 8090 | 獨立 nginx 容器 | `docker compose build web` |

兩者共用同一份 `frontend/src/`，差別只在 Vite 的 base path 與輸出目錄
（`vite.config.js` vs `vite.config.web.js`），詳見「專案結構」一節。
...
| 端點 | URL | 說明 |
|------|-----|------|
| Swagger UI | http://localhost:8080/apidocs | ⚠️ 目前**無需驗證**即可瀏覽，會列出所有 API 與參數。正式對外前建議用 nginx 限制來源 IP |
| API 說明頁 | http://localhost:8080/docs/ | `app/docs/` 提供的靜態說明 |
| Nginx 健康檢查 | http://localhost:8080/nginx-health | 回傳 `ok` |
| Flask 健康檢查 | http://localhost:8080/ | 回傳 `ok`（api 容器的 healthcheck 打這支） |
| Nginx（HTTPS） | https://localhost:8443 | 啟用 SSL 模式時；埠號可用 `.env` 的 `HTTPS_PORT` 覆寫 |

埠號都可以在 `.env` 覆寫：`HTTP_PORT`（8080）、`HTTPS_PORT`（8443）、`WEB_PORT`（8090）。
8080 而不是 80 是因為原本跟另一個本機專案衝突。

### 資料庫

⚠️ 三個資料庫的埠號**只綁在 127.0.0.1**（見 `docker-compose.db.yml`）。
Docker 的 port 發佈會直接寫 iptables、繞過 ufw，綁 0.0.0.0 等於對整個網路開放 ——
而 MongoDB 目前沒有開 `--auth`。要從別台機器連請用 SSH tunnel，
不要改成 0.0.0.0；真的需要時用 `.env` 的 `DB_BIND_HOST` 覆寫。
...
├── docker-compose.web.yml              # 玩家/藍圖/戰利品前端（nginx + frontend/dist-web 靜態檔）
├── docker-compose.nginx.yml            # nginx（api 反向代理）
│
...
> `--build` 會執行 multi-stage build：`api` 容器在內部跑 `npm run build`（後台管理介面）
> 打包後再建置 Flask image；`web` 容器（見 `docker/Dockerfile.web`）則在內部跑
> `npm run build:web`（玩家/藍圖/戰利品前端）。兩邊都不需要在本機手動 build，
> `docker compose up -d --build` 會自動處理。之後只改 Python 程式碼時重啟即可；
> 只有異動 `frontend/`、`requirements.txt`、`Dockerfile` / `Dockerfile.web`
> 才需要再加 `--build`。
...
物品 / 載具 / 商品 / **製造藍圖**主檔由 `tasks/scdata_sync.py` 從社群 API 同步而來。

| 資源 | Collection | 筆數（4.10） | 說明 |
|---|---|---|---|
| `items` | `item_master` | ~4,700 | 物品規格、體積（庫存 SCU 計算用） |
| `vehicles` | `vehicle_master` | ~200 | 載具，含貨艙 SCU |
| `commodities` | `commodity_master` | ~100 | 商品與箱體規格 |
| `blueprints` | `blueprint_master` | ~1,600 | **製造配方**：材料、製作時間、拆解回收 |

### 製造藍圖（blueprint_master）

⚠️ **`blueprint_master` 跟 `blueprints` 是兩個不同的 collection，別搞混：**

| | `blueprint_master` | `blueprints` |
|---|---|---|
| 是什麼 | 遊戲裡存在哪些配方 | 某位玩家擁有哪張藍圖 |
| 來源 | Wiki API 同步 | 人填（玩家自助或後台） |
| 可否寫入 | 唯讀 | CRUD |
| 模型 | `src/models/item.py` 的 `BlueprintMaster` | `src/models/blueprint.py` 的 `Blueprint` |

兩者靠 `blueprints.blueprint_uuid` → `blueprint_master._id` 連起來。
**那個欄位允許為空** —— 玩家仍可自由輸入名稱（主檔還沒同步、或遊戲版本比主檔新時需要）。

抓取上有一個關鍵參數：列表端點預設把 `ingredients` / `dismantle_returns` /
`output` 留空（只給 `ingredient_count` 這種摘要），要加
`?include=ingredients,output,dismantle_returns` 才會帶完整配方。
這個設定在 `src/scdata.py` 的 `WIKI_QUERY_EXTRA`，**別拿掉** ——
沒有它就得逐筆打 1,606 次明細端點，同步請求數會從 33 變成 1,639。

`output_item_uuid` 直接對得上 `item_master._id`，所以「這張藍圖產出哪個物品」
「這個物品要什麼材料」都能 join 現有物品主檔。查詢端點：

| 端點 | 說明 |
|---|---|
| `GET /blueprint/master` | 配方列表，可依 `output_type` 過濾、`available=1` 只看預設可用 |
| `GET /blueprint/master/search?q=` | 名稱搜尋（中英文與 `key` 都比對），給 autocomplete |
| `GET /blueprint/master/types` | 所有產出物類型 |
| `GET /blueprint/master/<uuid>` | 單一配方完整內容（材料、拆解回收） |
| `GET /blueprint/master/for-item/<item_uuid>` | 做出這個物品的所有配方 |
...
```bash
docker compose --profile background up -d worker beat
```

**遊戲改版後手動觸發**比等排程實在，有兩種方式：

**方式一：直接在 worker 容器內跑（同步執行，跑完才會有輸出）**
...
丟給 Celery worker 背景執行，立刻回應不用等：

```bash
TOKEN=$(curl -s -X POST http://localhost:8090/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"你的密碼"}' | python3 -c "import sys,json;print(json.load(sys.stdin)['token'])")

curl -s -X POST http://localhost:8090/item/sync \
